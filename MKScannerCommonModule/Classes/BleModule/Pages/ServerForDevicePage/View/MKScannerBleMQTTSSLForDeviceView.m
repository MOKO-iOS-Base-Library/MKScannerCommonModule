//
//  MKScannerBleMQTTSSLForDeviceView.m
...
@implementation MKScannerBleMQTTSSLForDeviceViewModel
@end

@interface MKScannerBleMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKScannerBleMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate mk_scanner_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate mk_scanner_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate mk_scanner_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate mk_scanner_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate mk_scanner_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKScannerBleMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKScannerBleMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKScannerCommonModule", @"MKScannerBleMQTTSSLForDeviceView", @"mk_scanner_switchSelectedIcon.png") : LOADICON(@"MKScannerCommonModule", @"MKScannerBleMQTTSSLForDeviceView", @"mk_scanner_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKScannerCommonModule", @"MKScannerBleMQTTSSLForDeviceView", @"mk_scanner_switchUnselectedIcon.png") forState:UIControlStateNormal];
