//
//  MKScannerManageBleDevicesSearchButton.m
...
@implementation MKScannerManageBleDevicesSearchButtonModel
@end

@interface MKScannerManageBleDevicesSearchButton ()
...
@implementation MKScannerManageBleDevicesSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(mk_scanner_scanSearchButtonClearMethod)]) {
        [self.delegate mk_scanner_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(mk_scanner_scanSearchButtonMethod)]) {
        [self.delegate mk_scanner_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKScannerManageBleDevicesSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKScannerManageBleDevicesSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKScannerCommonModule", @"MKScannerManageBleDevicesSearchButton", @"mk_scanner_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKScannerCommonModule", @"MKScannerManageBleDevicesSearchButton", @"mk_scanner_clearButtonIcon.png") forState:UIControlStateNormal];
