//
//  MKScannerUserCredentialsView.m
...
@implementation MKScannerUserCredentialsViewModel
@end

@interface MKScannerUserCredentialsView ()
...
@implementation MKScannerUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKScannerUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKScannerUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate mk_scanner_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate mk_scanner_mqtt_userCredentials_passwordChanged:text];
            }
        };
