...
@implementation MKScannerMQTTSSLForAppViewModel
@end

@interface MKScannerMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKScannerMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate mk_scanner_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate mk_scanner_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate mk_scanner_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(mk_scanner_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate mk_scanner_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKScannerMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKScannerMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKScannerCommonModule", @"MKScannerMQTTSSLForAppView", @"mk_scanner_switchSelectedIcon.png") : LOADICON(@"MKScannerCommonModule", @"MKScannerMQTTSSLForAppView", @"mk_scanner_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKScannerCommonModule", @"MKScannerMQTTSSLForAppView", @"mk_scanner_switchUnselectedIcon.png") forState:UIControlStateNormal];
