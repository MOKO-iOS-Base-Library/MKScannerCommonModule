...
@interface MKScannerOTAController ()<UITableViewDelegate,
...
@property (nonatomic, strong)id <MKScannerOTAProtocol>protocol;

@end

@implementation MKScannerOTAController

- (void)dealloc {
    NSLog(@"MKScannerOTAController销毁");
}
...
- (void)mk_loraTextButtonCellSelected:(NSInteger)index
    ...
    if (index == 0) {
        ...
        self.protocol.otaType = dataListIndex;
        return;
    }
}
...
- (void)setupBlock {
    @weakify(self);
    self.protocol.receiveOTAResult = ^(NSInteger result) {
        @strongify(self);
        [self receiveOTAResult:result];
    };
}
- (void)receiveOTAResult:(NSInteger)result {
    [[MKHudManager share] hide];
    self.leftButton.enabled = YES;
    if (result == 1) {
...
- (void)loadSection0Datas {
    ...
    cellModel.dataListIndex = self.protocol.otaType;
