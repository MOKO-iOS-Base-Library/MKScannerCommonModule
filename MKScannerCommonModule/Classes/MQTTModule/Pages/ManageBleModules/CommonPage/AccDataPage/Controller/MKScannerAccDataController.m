...
@interface MKScannerAccDataController ()<MFMailComposeViewControllerDelegate,
...
@property (nonatomic, strong)id <MKScannerAccDataProtocol>protocol;

@end

@implementation MKScannerAccDataController

- (void)dealloc {
    NSLog(@"MKScannerAccDataController销毁");
}
...
- (void)mk_scanner_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    @weakify(self);
    [self.protocol notifyAccData:isOn
                        sucBlock:^{
        @strongify(self);
...
- (void)setupBlock {
    @weakify(self);
    self.protocol.receiveAccDataBlock = ^(NSString * _Nonnull xAxis, NSString * _Nonnull yAxis, NSString * _Nonnull zAxis) {
        @strongify(self);
        [self receiveAccDatas:xAxis yAxis:yAxis zAxis:zAxis];
    };
}

#pragma mark - Notes
- (void)receiveAccDatas:(NSString *)xAxis yAxis:(NSString *)yAxis zAxis:(NSString *)zAxis {
    NSString *dateString = [self.dateFormatter stringFromDate:[NSDate date]];
    NSString *x = [@" X-axis:" stringByAppendingString:[NSString stringWithFormat:@"%@",xAxis]];
    NSString *y = [@" Y-axis:" stringByAppendingString:[NSString stringWithFormat:@"%@",yAxis]];
    NSString *z = [@" Z-axis:" stringByAppendingString:[NSString stringWithFormat:@"%@",zAxis]];
