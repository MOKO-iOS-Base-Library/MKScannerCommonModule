...
@interface MKScannerBXPCController ()<UITableViewDelegate,
...
@property (nonatomic, strong)id <MKScannerBXPCProtocol>protocol;

@property (nonatomic, strong)NSDictionary *bxpStatusDic;

@end

@implementation MKScannerBXPCController

- (void)dealloc {
    NSLog(@"MKScannerBXPCController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Real time T&H data
        id <MKScannerRealTimeTHDataProtocol>protocol = self.protocol.realTimeTHProtocol;
        if (protocol) {
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Historical T&H data
        id <MKScannerBXPCHistoricalTHDataProtocol>protocol = self.protocol.historicalTHDataProtocol;
        if (protocol) {
            MKScannerBXPCHistoricalTHDataController *vc = [[MKScannerBXPCHistoricalTHDataController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
...
- (void)loadSection2Datas {
    ...
    NSString *hex = [NSString stringWithFormat:@"%1lx",(unsigned long)[self.protocol.deviceBleInfo[@"data"][@"sensor_status"] integerValue]];
    if (hex.length == 1) {
        hex = [@"0" stringByAppendingString:hex];
    }
    
    NSString *binary = [hex binaryByhex];
