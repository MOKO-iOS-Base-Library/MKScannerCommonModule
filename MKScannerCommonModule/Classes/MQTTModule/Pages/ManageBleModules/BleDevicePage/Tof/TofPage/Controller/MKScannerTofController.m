...
@interface MKScannerTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)id <MKScannerTofProtocol>protocol;

@property (nonatomic, strong)NSDictionary *bxpStatusDic;

@end

@implementation MKScannerTofController

- (void)dealloc {
    NSLog(@"MKScannerTofController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Tof sensor data
        id <MKScannerTofSensorDataProtocol>protocol = self.protocol.sensorDataProtocol;
        if (protocol) {
            MKScannerTofSensorDataController *vc = [[MKScannerTofSensorDataController alloc] initWithProtocol:protocol];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Accelerometer data
        id <MKScannerAccDataProtocol>protocol = self.protocol.accDataProtocol;
        if (protocol) {
    ...
    if (indexPath.section == 3 && indexPath.row == 2) {
        ...
        id <MKScannerTofSensorParamsProtocol>protocol = self.protocol.sensorParamsProtocol;
        if (protocol) {
            MKScannerTofSensorParamsController *vc = [[MKScannerTofSensorParamsController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
    ...
    if (indexPath.section == 3 && indexPath.row == 3) {
        ...
        id <MKScannerTofAdvParamsProtocol>protocol = self.protocol.advProtocol;
        if (protocol) {
            MKScannerTofAdvParamsController *vc = [[MKScannerTofAdvParamsController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
