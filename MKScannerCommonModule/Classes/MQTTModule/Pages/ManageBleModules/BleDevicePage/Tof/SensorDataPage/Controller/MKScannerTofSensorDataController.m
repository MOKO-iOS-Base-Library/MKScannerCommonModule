//
//  MKScannerTofSensorDataController.m
...
@interface MKScannerTofSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@property (nonatomic, strong)id <MKScannerTofSensorDataProtocol>protocol;

@end

@implementation MKScannerTofSensorDataController

- (void)dealloc {
    NSLog(@"MKScannerTofSensorDataController销毁");
}
...
- (void)setupBlock {
    @weakify(self);
    self.protocol.receiveSensorDataBlock = ^(NSString * _Nonnull distance) {
        @strongify(self);
        [self receiveSensorDatas:distance];
    };
}
- (void)receiveSensorDatas:(NSString *)distance {
