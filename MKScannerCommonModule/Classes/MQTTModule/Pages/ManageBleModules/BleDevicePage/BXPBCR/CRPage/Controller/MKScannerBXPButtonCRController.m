//
//  MKScannerBXPButtonCRController.m
...
@interface MKScannerBXPButtonCRController ()<UITableViewDelegate,
...
@property (nonatomic, strong)id <MKScannerBXPButtonCRProtocol>protocol;

@property (nonatomic, strong)MKScannerBXPButtonCRReadModel *readModel;

@property (nonatomic, assign)NSInteger batteryMode;

@end

@implementation MKScannerBXPButtonCRController

- (void)dealloc {
    NSLog(@"MKScannerBXPButtonCRController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 7 && indexPath.row == 1) {
        //Alarm Event
        id <MKScannerBXPButtonCRAlarmEventProtocol>protocol = self.protocol.alarmEventProtocol;
        if (protocol) {
            MKScannerBXPButtonCRAlarmEventController *vc = [[MKScannerBXPButtonCRAlarmEventController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 8) {
        return (self.protocol.supportBatteryMode ? self.section8List.count : 0);
    }
...
- (void)mk_scanner_pressEventCountCell_clearButtonPressed:(NSInteger)index {
    ...
    @weakify(self);
    [self.protocol clearEventCountWithType:index
                                  sucBlock:^{
        @strongify(self);
...
- (void)dismissAlarmStatus {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    @weakify(self);
    [self.protocol dismissBXPButtonCRAlarmStatusWithSucBlock:^(id  _Nonnull returnData) {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)updateStatusDatas {
    ...
    if (self.protocol.supportBatteryMode) {
        MKTextButtonCellModel *cellModel5 = self.section8List[0];
        cellModel5.dataListIndex = self.batteryMode;
    }
