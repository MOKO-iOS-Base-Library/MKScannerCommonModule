//
//  MKScannerPirSensorDataController.m
...
@interface MKScannerPirSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@property (nonatomic, strong)id <MKScannerPirSensorDataProtocol>protocol;

@end

@implementation MKScannerPirSensorDataController

- (void)dealloc {
    NSLog(@"MKScannerPirSensorDataController销毁");
}
...
- (void)mk_scanner_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    @weakify(self);
    [self.protocol notifySensorData:isOn
                           sucBlock:^{
        @strongify(self);
...
- (void)setupBlock {
    @weakify(self);
    self.protocol.receiveSensorDataBlock = ^(NSString * _Nonnull doorState, NSString * _Nonnull pirState) {
        @strongify(self);
        [self receiveSensorDatas:doorState pirState:pirState];
    };
}
- (void)receiveSensorDatas:(NSString *)doorState pirState:(NSString *)pirState {
