...
@interface MKScannerBXPTController ()<UITableViewDelegate,
...
@property (nonatomic, strong)id <MKScannerBXPTProtocol>protocol;

@property (nonatomic, strong)NSDictionary *bxpStatusDic;

@end

@implementation MKScannerBXPTController

- (void)dealloc {
    NSLog(@"MKScannerBXPTController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Remote reminder
        id <MKScannerBXPSReminderProtocol>protocol = self.protocol.reminderProtocol;
        if (protocol) {
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Motion event data
        id <MKScannerBXPTMotionEventProtocol>protocol = self.protocol.eventProtocol;
        if (protocol) {
            MKScannerBXPTMotionEventController *vc = [[MKScannerBXPTMotionEventController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
    ...
    if (indexPath.section == 3 && indexPath.row == 3) {
        ...
        id <MKScannerBXPTAccParamsProtocol>protocol = self.protocol.accParamsProtocol;
        if (protocol) {
            MKScannerBXPTAccParamsController *vc = [[MKScannerBXPTAccParamsController alloc] initWithProtocol:protocol];
            [self.navigationController pushViewController:vc animated:YES];
        } else {
            [self.view showCentralToast:@"Protocol cannot be empty"];
        }
